# ADR: React Bridge — PreactWrapperV1 vs preact/compat
...
> 「Preact 部品を PHP/JS でも React/TS でも使えるようにするだけなら preact/compat を使えば、こんなに回りくどいことをしなくても簡単だったのではないか」

本 ADR はこの批評をスチールマンしつつ、**(a)** 現行は妥当解か、**(b)** 将来どんな状況になったら別案へ移行すべきかを記録する。
...
→ 批評者が念頭に置いていたのは **P1** の可能性が高い。これがスチールマンの主軸。
...
## 4. Decision — 現条件下での Pareto 最適

5 制約 + C6 軽量依存 + C5 AI_BBS 対称性を **同時に** 満たす案は C のみ。A は C2/C3 を残し、B は C5 を破壊し、D は移植コストが過大、E はメンテ二重化が即破綻。
...
各トリガが起きたら本 ADR を **Reconsider** ステータスへ書き換える。
