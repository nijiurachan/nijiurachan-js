...
- 静的画像を出すときは、oejsに描き込み、baseformは触らない
- アニメ画像を出すときは、baseformにapngファイルをBase64にしたものを設定する
    - oejsは削除(！)してプレビューするimgタグに差し替える

(！) oejsを消すのはおそらく、本家掲示板ではoejsを置いたままだと本家の処理が働いてbaseformが静的画像で上書きされてしまうため
...
- oebtnjとftblを非表示で置いておき、はっちゃんが押すのを待つ
- oebtnjが押されたら、ftbl下にoejsとbaseformを配置して非表示を解除する
- 投稿時、oejsとbaseformのどちらかをupfileに取り込む([後述](#投稿))

## 添付ボタンとお絵描きの兼ね合い

"添付済ですが上書きしますか？"のようなメッセージを出したくないので、次のようにモードを設けて
アクノスペイントとはっちゃんを排他する。
ツールを切り替える前にクリアボタンを押してリセットしてもらう形。
